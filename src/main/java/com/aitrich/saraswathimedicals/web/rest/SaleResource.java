...
/**
 * REST controller for managing {@link com.aitrich.saraswathimedicals.domain.Sale}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class SaleResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/sales/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<Sale> partialUpdateSale(@PathVariable(value = "id", required = false) final Long id, @RequestBody Sale sale)
        ...
        Optional<Sale> result = saleRepository
            ...
            .map(
                existingSale -> {
                    if (sale.getToatal() != null) {
                        existingSale.setToatal(sale.getToatal());
                    }
