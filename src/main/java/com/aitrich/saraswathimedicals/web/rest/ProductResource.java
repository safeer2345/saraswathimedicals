...
/**
 * REST controller for managing {@link com.aitrich.saraswathimedicals.domain.Product}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ProductResource {
    ...
    public List<Product> getAllProducts(@RequestParam(required = false) String filter) {
        ...
        if ("productrate-is-null".equals(filter)) {
            log.debug("REST request to get all Products where productRate is null");
            return StreamSupport
                .stream(productRepository.findAll().spliterator(), false)
                .filter(product -> product.getProductRate() == null)
                .collect(Collectors.toList());
        }
